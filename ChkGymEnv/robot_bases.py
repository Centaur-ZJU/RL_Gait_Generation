  ...
  def reset(self, bullet_client):
    ...
    if (self.doneLoading == 0):
      ...
      if self.self_collision:
        self.objects = self._p.loadMJCF(os.path.join(currentdir , 'data', 
                                                     self.model_xml),
        ...
        self.objects = self._p.loadMJCF(
            os.path.join(currentdir , 'data', self.model_xml, flags = pybullet.URDF_GOOGLEY_UNDEFINED_COLORS))
        self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
            self._p, self.objects)
...
class BodyPart:
  ...
  def apply_external_force(self, force):
    self._p.applyExternalForce(self.bodies[self.bodyIndex], self.bodyPartIndex, force, 
