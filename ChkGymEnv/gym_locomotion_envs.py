...
import numpy as np
import pybullet
from robot_locomotors import ChkHumanoid
...
class WalkerBaseBulletEnv(MJCFBaseBulletEnv):
  ...
  def __init__(self, robot, render=False):
    ...
    self.stateId = -1
    # self.tired = np.zeros((self.robot.action_dim,))    # 每个关节的疲劳程度
  ...
  def reset(self):
    ...
    # self.tired = np.zeros((self.robot.action_dim,))
    self.steps = 0
  ...
  def step(self, a):
    ...
    # feet_on_ground_concurrently = -10 * (sum(self.robot.feet_contact)>1.5)  # 双脚同时着地惩罚,傻子一样单脚跳
    # self.tired = 0.9 * self.tired + 0.1 * np.abs(a)   # tired 的期望值为 a 的期望值的 1 倍
    ...
    # return state, self._alive, bool(done), {}
...
class ChkHumanoidBulletEnv(WalkerBaseBulletEnv):

  def __init__(self, robot=None, render=False):
    if robot is None:
      self.robot = ChkHumanoid()
    else:
      self.robot = robot
