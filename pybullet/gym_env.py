...
if __name__ == '__main__':
    ...
    eg = ExperimentGrid(name=time.strftime("%Y-%m-%d_",time.localtime())+'ppo')
    eg.add('env_name', 'HumanoidBulletEnv-v0', '', True)
    eg.add('seed', [7*i for i in range(1, 1+args.num_runs)])
    eg.add('epochs', 10000)
